...
for cle in liste_cle:
    ...
    for path_corpus in glob.glob(f"{path_data}/*"):
        ...
        for path in glob.glob(f"{path_corpus}/*OCR/*/NER*/SIM/{calc}*.json"):##NER
            if "LOC" in path or "MISC" in path or "PER" in path or "ORG" in path:
                continue
            p = Path(path)
            print("Path :", p)
            corpus = p.parts[1]
        ...
        # # # ##____Pour la NER_____________________
        bm_path_ner = p.parent.parent.parent / "Boite_moustache" / nommage_version  ##Pour NER
