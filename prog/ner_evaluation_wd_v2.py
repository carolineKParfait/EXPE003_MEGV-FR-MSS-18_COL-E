...
def lire_dico(fichier_json, label):
    liste_en = []
    for key, value in fichier_json.items():
        if not isinstance(value, dict):
            continue
        if value.get("label") == label and "text" in value:
            liste_en.append(value["text"])

    if not liste_en:
        # Aucun texte avec ce label, on ne retourne rien
        print(f"→ Aucun texte trouvé avec le label '{label}'")
        return  # pas de return de texte

    chaien = " ".join(liste_en)
    print("Chaine EN :", chaien)
    return chaien


def traiter_corpus(reference_files,ocr_files, lab, is_json=False):
    ...
    for reference_file in reference_files:
        ...
        if is_json == True:
            ner_res = lire_dico(texte_ref, lab)
            if ner_res:  # on ne traite que si on a trouvé quelque chose
                print("Résultats REF : ", ner_res)
            else:
                print(f"→ Aucun label {lab} trouvé dans ce fichier REF, on passe")
        ...
        for ocr_file in ocr_files:
            ...
            # Pour l'eval sur la REN
            if is_json == True:
                ner_res_ocr = lire_dico(texte_ocr, lab)
                if ner_res_ocr:
        ...
        #
            ...
            if ner_res_ocr != None and ner_res != None and len(ner_res) > 1 and len(ner_res_ocr) > 1:
                ...
                if name_ref_file == name_ocr_file and configNER == configNER_ocr:## pour NER
                    ...
                    json_path = (sim_path / f"sim2-3_{name_ocr_file}_{model_name_ocr}_{configNER_ocr}_{lab}.json").resolve() ## pour NER # absolu
