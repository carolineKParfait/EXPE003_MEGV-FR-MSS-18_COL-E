...
for cle in liste_cle:
    ...
    for path_corpus in glob.glob(f"{path_data}/*"):
        ...
        for path in glob.glob(f"{path_corpus}/*OCR/*/NER*/SIM/{calc}*{label}.json"):##NER
            p = Path(path)
            print("Path :", p)
            corpus = p.parts[1]
            print("Corpus :", corpus)
            ...
            #____Pour la NER_____________________
            decoup = p.parts[-1]
            version=decoup.split("_")[-3] ## -2 lorsque tous label confondus
            print("Version : ",version)
            # vers_ren = p.parts[-1]
            vers_ren = Path(decoup.split("_")[-2]).stem ## -1 lorsque tous label confondus
            print("Version de NER : ",vers_ren)
            cat_label = Path(decoup.split("_")[-1]).stem ## Commenter lorsque tous label confondus
            print("Label : ", cat_label)
            #____Pour la NER_____________________
        ...
        bm_path = chemin_stockage(bm_path_ner) ## ______________________> adapter selon NER ou Texte
        print("Chemin de sortie : ",f"{bm_path}/{calc}_{sous_corpus}_{version}_{label}_{cle}.png")
        # plt.show()
    ...
    # #
    # # # ##____Pour la NER_____________________
        plt.savefig(f"{bm_path}/{calc}_{sous_corpus}_{version}_{label}_{cle}.png",dpi=300, bbox_inches="tight")##NER
    # # ##____Pour la NER_____________________
    # # plt.close()
